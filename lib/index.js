        ...
        try {
            ...
            const entryRequestSplitPath = (() => {
                if (config.entry.requestSplit) {
                    return path_1.default.relative(process.cwd(), config.entry.requestSplit);
                }
                else {
                    return null;
                }
            ...
            if (entryRequestSplitPath) {
                ...
                for (let pathsKey in paths) {
                    ...
                    for (let method in path) {
                        ...
                        const requestRendered = await ejs_1.renderFile(entryRequestSplitPath, {
                            definitionsRelPath,
                            pathsKey,
                            method,
                            raw
                        });
                        fs_1.writeFileSync(config.output.requestSplit.replace('{{name}}', raw.operationId), prettier_1.format(requestRendered, prettierOptions), 'utf-8');
                    }
                }
            }
